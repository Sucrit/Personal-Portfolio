import React from 'react';

const PhilippinesFlag: React.FC<{ style?: React.CSSProperties, className?: string }> = ({ style, className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="12"
    viewBox="-28 -45 180 90"
    fill="#fcd116"
    style={style}
    className={className}
    aria-label="Flag of the Philippines"
  >
    <title>Flag of the Philippines</title>
    <path fill="#0038a8" d="m-28-45h180V0L0 44z"/>
    <path fill="#ce1126" d="m0 0h152v45H-28z"/>
    <path fill="#fff" d="m49.942 0L-28 45v-90z"/>
    <circle r="9"/>
    <g id="c">
      <g id="b">
        <path id="a" d="m0 0-3.164-15.909.945-.946zl-1.169-17.831L0-19l1.169 1.169zl2.219-16.855.945.946z"/>
        <use transform="scale(-1)" xlinkHref="#a"/>
      </g>
      <use transform="rotate(90)" xlinkHref="#b"/>
    </g>
    <use transform="rotate(45)" xlinkHref="#c"/>
    <path id="s" d="m40.942 0-3.382 1.176L37.487 4.755l-2.163-2.853-3.427 1.037L33.942 0l-2.045-2.939 3.427 1.037 2.163-2.853L37.56-1.176z"/>
    <use transform="rotate(120,-2.019,0)" xlinkHref="#s"/>
    <use transform="rotate(240,-2.019,0)" xlinkHref="#s"/>
  </svg>
);

export default PhilippinesFlag;
